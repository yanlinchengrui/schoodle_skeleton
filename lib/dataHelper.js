...
module.exports = function makeDataHelper(db) {
  return {
          ...
          .then ((result) => {
            ...
            let winDate = null;
              if (result[0].count1 >= result[0].votes_to_win) {
                winDate = result[0].date_1
              } else if(result[0].count2 >= result[0].votes_to_win){
                winDate = result[0].date_2
              } else if(result[0].count3 >= result[0].votes_to_win){
                winDate = result[0].date_3
              }else if(result[0].count4 >= result[0].votes_to_win){
                winDate = result[0].date_4
              }else if(result[0].count5 >= result[0].votes_to_win){
                winDate = result[0].date_5
              }
              if(winDate){
    ...
    updateParticipantNameByEmail: (email, name) => {
      return db('participants')
              .where('email', email)
              .update('name', name);
    },
